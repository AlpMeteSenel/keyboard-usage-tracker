...
fn default_stats() -> DashboardStats {
    DashboardStats {
        total_keys: 0, total_clicks: 0, keys_today: 0, clicks_today: 0,
        current_wpm: 0.0, avg_wpm: 0.0, best_wpm: 0.0, active_minutes_today: 0,
        top_keys: vec![], held_keys: vec![], recent: vec![],
        hourly_activity: vec![], daily_activity: vec![], weekly_activity: vec![],
        monthly_activity: vec![], wpm_timeline: vec![],
        period_stats: PeriodStats {
            this_hour: PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
            today: PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
            this_week: PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
            this_month: PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
            all_time: PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
        },
    }
}
...
fn query_stats(db_path: &PathBuf) -> DashboardStats {
    ...
    let top_keys: Vec<KeyCount> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let held_keys: Vec<HeldKeyStat> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let hourly_activity: Vec<HourlyActivity> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let daily_activity: Vec<DailyActivity> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let weekly_activity: Vec<WeeklyActivity> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let monthly_activity: Vec<MonthlyActivity> = if let Ok(mut stmt) = conn
        .prepare(
    ...
    let period_stats = PeriodStats {
        this_hour: period_stat(
            "timestamp >= strftime('%Y-%m-%dT%H:00:00','now','localtime')",
        ),
        today: period_stat(
            "timestamp >= strftime('%Y-%m-%dT00:00:00','now','localtime')",
        ),
        this_week: period_stat(
            "timestamp >= strftime('%Y-%m-%dT00:00:00','now','localtime','weekday 0','-7 days')",
        ),
        this_month: period_stat(
            "timestamp >= strftime('%Y-%m-01T00:00:00','now','localtime')",
        ),
...
fn query_hour_stats(db_path: &PathBuf, offset: u32) -> PeriodStat {
    ...
    let conn = match Connection::open(db_path) {
        Ok(c) => c,
        Err(_) => return PeriodStat { keys: 0, clicks: 0, wpm: 0.0, active_minutes: 0 },
    };
...
pub fn start_dashboard(db_path: Arc<PathBuf>) {
    ...
    thread::spawn(move || {
        ...
        let json_header: tiny_http::Header = "Content-Type: application/json"
            .parse()
            .unwrap();
        let html_header: tiny_http::Header = "Content-Type: text/html; charset=utf-8"
            .parse()
            .unwrap();
        let ico_header: tiny_http::Header = "Content-Type: image/x-icon"
            .parse()
            .unwrap();
        for request in server.incoming_requests() {
            ...
            match path {
                "/favicon.ico" => {
                    let response = tiny_http::Response::from_data(FAVICON_ICO)
                        .with_header(ico_header.clone());
                ...
                "/api/stats" => {
                    ...
                    let json = stats_cache.read().unwrap_or_else(|e| e.into_inner()).clone();
                    let response = tiny_http::Response::from_string(json)
                        .with_header(json_header.clone());
                    let _ = request.respond(response);
                }
